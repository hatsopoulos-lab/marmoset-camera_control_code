...
class params: 
    frameRate = 20
    exposure = 5000.0 # in microseconds. Increasing exposure will allow more light in but also increase motion blur
    gain = 10 # increase for brighter images, decrease for darker
    nCams = 4
...
def image_thread(cam, cam_num, timestamps, filename): #+++
    ...
    while record:
        try:
            ...
            image_filename = '%s/cam%d/%s_%s_%s_%s_cam%d_event_%s_frame_%s.jpg' % (filename.image_folder, 
                                                                                   cam_num, 
                                                                                   filename.marms, 
                                                                                   filename.date, 
                                                                                   filename.expName, 
...
def main():
    ...
    for cam_num in range(params.nCams): 
        file_path = '%s/cam%d' %(filename.image_folder, cam_num+1)
...
if __name__ == '__main__':
    ...
    class filename: 
        ...
        if type(session) == str:
            image_folder = '/media/marms/storage/%s/%s/%s/%s'        % (expName, marms, date, session)
        else:
            image_folder = '/media/marms/storage/%s/%s/%s/session%d' % (expName, marms, date, session)
    main()
