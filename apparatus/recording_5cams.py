...
def image_thread(cam, cam_num, timestamps, filename): #+++
    ...
    while record:
        try:
            ...
            if cam_num <= params.hardDrive_camNum_separator:
                image_folder = filename.image_folder1
            elif cam_num == params.nCams and cam_num % 2 == 1:
                if frameNum % 2 == 1:
                    image_folder = filename.image_folder1
                else:
                    image_folder = filename.image_folder2
            else:
                image_folder = filename.image_folder2
            
            image_filename = '%s/raw_cam%d/%s_%s_%s_session_%d_cam%d_event_%s_frame_%s_currentTime_%d_%s.raw' % (image_folder, 
                                                                                                                 cam_num, 
                                                                                                                 filename.marms, 
                                                                                                                 filename.date, 
                                                                                                                 filename.expName, 
...
def main():
    ...
    for cam_num in range(0, params.nCams): 
        if cam_num+1 <= params.hardDrive_camNum_separator:
            image_folder = filename.image_folder1
        else:
            image_folder = filename.image_folder2
            
        file_path = '%s/raw_cam%d' %(image_folder, cam_num+1)
        if not os.path.isdir(file_path): 
            os.makedirs(file_path)

        if cam_num + 1 == params.nCams and (cam_num+1) % 2 == 1:
            file_path = '%s/raw_cam%d' %(filename.image_folder1, cam_num+1)
            print(file_path)
            if not os.path.isdir(file_path): 
                os.makedirs(file_path)            
    ...
    for cam in range(0, num_cameras): 
        ...
        elif cam_list.GetByIndex(cam).TLDevice.DeviceSerialNumber() == params.cam5:
            cam5 = cam_list.GetByIndex(cam)
